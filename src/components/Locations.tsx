...
import { useFeaturedLocations, LocationItem } from "@/hooks/useLocations";
...
const Locations = () => {
  ...
  const { locations: featuredLocations } = useFeaturedLocations();
  ...
  const renderCard = (location: LocationItem) => (
    <Link to={`/location/${location.slug}`} className="block">
      ...
      <div className="p-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <span className="text-xl font-light text-foreground">PKR{location.price}</span>
